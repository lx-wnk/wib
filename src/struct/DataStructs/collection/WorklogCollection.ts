...
export default class WorklogCollection extends AbstractCollection {
  ...
  public getPrintData(): object {
    return this.getCalculatedPrintData((new StartStruct().fromSavedData().time.getTime()));
  }

  public getCalculatedPrintData(startTime: number, order = WorklogCollection.possibleOrderKeys.time): object {
    ...
    keySortedEntries = this.getKeySortedData(timeSortedEntries, new Date(startTime), order);
  ...
  private getKeySortedData(timeSortedEntries: object, startTime: Date, order: string): object {
    ...
    if (!startTime && 0 < Object.keys(timeSortedEntries).length) {
      startTime = new Date(timeSortedEntries[Object.keys(timeSortedEntries)[0]].time);
    }
  ...
  public first(): WorklogStruct | null {
    ...
    if (!Object.values(this.entries).shift().deleted) {
      return Object.values(this.entries).shift();
    }

    for (const key in entries) {
      if (!entries[key].deleted) {
        return entries[key];
      }
    }
