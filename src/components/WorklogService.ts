...
export class WorklogService {
    ...
    public async create(commandValues: string[], unexpected: string, time: Date = new Date()): Promise<WorklogEntity> {
        ...
        worklog.iterator = (await this.worklogRepository.getUndeletedByDateIterator(currentDay.start)).length + 1;
    ...
    public update(iterator: number, commandValues: string[], unexpected: string, time?: Date): void {
        this.worklogRepository
            .getUndeletedByDateIterator(new Date(), iterator)
            .then((result) => {
                if (result.length !== 1) {
        ...
        this.worklogRepository
            ...
            .then((result) => {
                if (result.length !== 1) {
                    ...
                    console.error(
                        'Error: Failed to delete worklog with iterator ' +
                            iterator +
                            '. Found ' +
                            result.length +
                            ' records.'
